...
#-------------------------------#    
    def capturar(self, datos):
        ...
        try:
            ...
            insertAlumno = "INSERT INTO Cursa VALUES("+cursadp.toStringSql()+")"
...
##        return datos
#-------------------------------#
    def consultarCC(self,cc):
        # 1. Abrir el archivo
        # archivoIn = open("Clientes.txt","r")
        conexion = mysql.connector.connect(user="root", database="Universidad")
        
        # Preparar query y ejecutarlo
        query = "SELECT * FROM Cursa WHERE cveCurso='"+cc+"'"
        statement = conexion.cursor()
        statement.execute(query)
        ...
        #if encontrado == False:
        if (not encontrado):
            datos = "No se localizaron alumnos en "+cc
